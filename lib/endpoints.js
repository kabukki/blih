 ...
 */
{
    ...
    method: 'GET',
    path: _ => '/repositories',
    transform: data => Object.keys(data.repositories).filter(r => r.length).sort().map(r => ({
        name: r,
    ...
    path: r => `/repository/${r}/acls`,
    method: 'GET',
    transform: data => Object.keys(data).filter(c => c.length && data[c].length).sort().map(c => ({
        name: c,
        rights: data[c]
    })),
    onError: _ => []
    ...
    path: _ => '/sshkeys',
    method: 'GET',
    transform: data => Object.keys(data).filter(k => k.length).sort().map(k => ({
        name: k,
        data: data[k]
    })),
    onError: _ => []
